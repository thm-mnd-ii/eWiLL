    ...
    override fun eval(diagramEvalPL: DiagramEvalPL, rule: Rule): RuleEvalResult {
        ...
        for(diagramConnection in diagramEvalPL.bestSolutionDiagram.connections){
            if(diagramConnection.from.otherModelNode == null || diagramConnection.to.otherModelNode == null){
                messages.add(ResultMessage(FeedbackLevel.DEBUG,"Connection can be skipped, because one of the entities is missing.",diagramConnection.from.entity!!.id!!,"",StatusLevel.INCORRECT,ResultMessageType.Connection,diagramConnection.to.entity!!.id!!))
                errors++
                continue
            }
            if(diagramConnection.to.entity!!.left!! < diagramConnection.from.entity!!.left!!){
                messages.add(ResultMessage(FeedbackLevel.BASIC,"Entity FROM ${diagramConnection.to.entity!!.entityName} is connected TO the left to ${diagramConnection.from.entity!!.entityName}  ",diagramConnection.from.entity!!.id!!,"",StatusLevel.INCORRECT,ResultMessageType.Connection,diagramConnection.to.entity!!.id!!))
                errors++
            }

            var connection = findConnection(diagramConnection,diagramEvalPL)
            if(connection != null){
                messages.add(ResultMessage(FeedbackLevel.DEBUG,"Connection ${diagramConnection.from.entity!!.entityName} to ${diagramConnection.to.entity!!.entityName} is present ",diagramConnection.from.entity!!.id!!,"",StatusLevel.CORRECT,ResultMessageType.Connection,connection.to.entity!!.id!!))

                if(connection.type != diagramConnection.type){
                    messages.add(ResultMessage(FeedbackLevel.BASIC,"Connection ${diagramConnection.from.entity!!.entityName} to ${diagramConnection.to.entity!!.entityName} has wrong type ",diagramConnection.from.entity!!.id!!,"",StatusLevel.INCORRECT,ResultMessageType.Connection,connection.to.entity!!.id!!))
                    messages.add(ResultMessage(FeedbackLevel.INFO,"Correct type is ${connection.type}",diagramConnection.from.entity!!.id!!,"",
