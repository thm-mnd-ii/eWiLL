...
<template>
  <div>
    ...
    <DialogImportExportDiagramVue ref="dialogImportExport"></DialogImportExportDiagramVue>

    <v-card :elevation="0" class="explorer">
      <v-divider></v-divider>
      <v-card-actions class="btn-menue">
        <div>
          <v-btn icon class="explorerBtn" :disabled="deleteActive" @click="deleteCurrentDiagram">
            <v-icon id="saveAsNewBtn">mdi-new-box</v-icon>
            <v-tooltip activator="parent" location="end">Neues Diagram</v-tooltip>
          </v-btn>
        </div>
        <div>
          <v-btn icon="mdi-content-save" class="explorerBtn" :disabled="deleteActive" @click="saveDialogButtonClick">
            <v-icon id="saveAsNewBtn">mdi-content-save</v-icon>
            <v-tooltip activator="parent" location="end">Speichern</v-tooltip>
          </v-btn>
        </div>
        <div>
          <v-btn icon="mdi-import" class="explorerBtn" :disabled="deleteActive" @click="openImportExportDialog">
            <v-icon id="saveAsNewBtn">mdi-import</v-icon>
            <v-tooltip activator="parent" location="end">Import/Export</v-tooltip>
          </v-btn>
        </div>

        <div>
          <v-btn icon="mdi-undo" class="explorerBtn" :disabled="deleteActive || diagramStore.historyIndex <= 1" @click="diagramStore.undo">
            <v-icon id="saveAsNewBtn">mdi-undo</v-icon>
            <v-tooltip activator="parent" location="end">Rückgängig</v-tooltip>
          </v-btn>
        </div>
        <div>
          <v-btn icon="mdi-redo" class="explorerBtn" :disabled="deleteActive || diagramStore.historyIndex >= diagramStore.history.length" @click="diagramStore.redo">
            <v-icon id="saveAsNewBtn">mdi-redo</v-icon>
            <v-tooltip activator="parent" location="end">Wiederholen</v-tooltip>
          </v-btn>
        </div>
        <div>
          <v-btn icon="mdi-delete" class="explorerBtn" :class="{ deleteBtnActive: deleteActive }" @click="deleteActive = !deleteActive">
            <v-icon id="saveAsNewBtn">mdi-delete</v-icon>
            <v-tooltip activator="parent" location="end">Löschen</v-tooltip>
          </v-btn>
        </div>
        <div>
          <v-btn icon="mdi-arrow-left" class="explorerBtn" :disabled="deleteActive || categoriesViewActive" @click="moveToOverview">
            <v-icon id="saveAsNewBtn">mdi-arrow-left</v-icon>
            <v-tooltip activator="parent" location="end">Zurück</v-tooltip>
          </v-btn>
        </div>
        <!-- reload -->
        <div>
          <v-btn icon="mdi-refresh" class="explorerBtn" :disabled="deleteActive" @click="updateFiles">
            <v-icon id="saveAsNewBtn">mdi-refresh</v-icon>
            <v-tooltip activator="parent" location="end">Dateien Aktualisieren</v-tooltip>
          </v-btn>
        </div>
      </v-card-actions>
...
import DialogImportExportDiagramVue from '../dialog/DialogImportExportDiagram.vue'
...
const dialogImportExport = ref<typeof DialogImportExportDiagramVue>()
...
const openImportExportDialog = () => {
  dialogImportExport.value?.openDialog().then(() => {
    updateFiles()
