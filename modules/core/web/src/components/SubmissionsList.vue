  ...
  <div class="container">
    ...
    <v-data-table :headers="headers" :items="listItems" class="elevation-1" density="default" height="480px" :search="search">
      ...
      <template #[`item.result.score`]="{ item }">
        <v-chip b-tooltip.hover class="fixed-size-chip" title="Zeige komplettes Feedback" :style="{ background: colors(item), color: 'black' }" @click="openFeedback(item.result.comments, item.submission.attempt)"> {{ item.result.score.toFixed(2) }}</v-chip>
      </template>
...
import type SubmissionListItem from '@/model/submission/SubmissionsListItem'
