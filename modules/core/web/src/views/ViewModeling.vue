  ...
  <div class="container">
    ...
    <div class="navigation">
      <v-system-bar color="white" elevation="1" window>
        ...
        <span v-if="autosave">
          <span>Dein Diagramm <b>{{ diagramStore.diagram.name }}</b> wurde automatisch um {{ lastAutosaveTime }} gespeichert</span>
        </span>
...
watch(() => [diagramStore.diagram.id, diagramStore.diagram.entities.length], ([id, entities]) => {
  if (id === 0 && entities === 1) {
...
watch(() => diagramStore.diagram.id, (id) => {
  if (id !== 0) {
    _setAutosaveTimer();
    _clearTimer(snackbarTimer);
  } else if (id === 0) {
